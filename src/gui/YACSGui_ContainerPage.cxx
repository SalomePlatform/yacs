...
{
  DEBTRACE("YACSGui_ContainerPage::setDefinitionName " << theName.latin1());
  // --- TODO: create a command in hmi for container rename.
  bool ret = false;
  ...
  if (!alreadyExists)
  {
    ...
    aContext->getProc()->containerMap.erase(oldName);
    aContext->getProc()->containerMap[theName.latin1()] = getContainer();
...
QString YACSGui_ContainerPage::getWorkingDir() const
{
  return ( getSalomeContainer() ? QString(getSalomeContainer()->_params.workingdir) : QString("") );
}

void YACSGui_ContainerPage::setWorkingDir( const QString& theWD)
{
  if ( getSalomeContainer() )
    getSalomeContainer()->setProperty( "workingdir", theWD.latin1() );
}
...
void YACSGui_ContainerPage::checkModifications( bool& theWarnToShow, bool& theToApply )
{
  ...
  if ( myDefinitionName->text().compare(getDefinitionName()) != 0
       ...
       ||
       myNbComponentNodes->value() != getNbComponentNodes() 
       ||
       myWorkingDir->text().compare(getWorkingDir()) != 0
     ) isModified = true;
...
void YACSGui_ContainerPage::updateState()
{
  ...
  if ( myWorkingDir )
    myWorkingDir->setText(getWorkingDir());
