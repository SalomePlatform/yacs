...
{
  DEBTRACE("YACSGui_Table::setCellType " << indR << " " << indC);
...
{
  DEBTRACE("YACSGui_Table::createItem " << row << " " << col);
...
{
  DEBTRACE("YACSGui_Table::strings " << col << " " << numRows());
  mat.clear();
  for( int i=0; i<numRows(); i++ )
  {
    ( ( YACSGui_Table* )this )->endEdit( i, col, false, false );
    QTableItem* it = item( i, col );
    QString val=it ? it->text() : QString::null;
#ifdef _DEVDEBUG_
    if(!val.isNull())
...
{
  //DEBTRACE("col="<<col<<" numcols="<<numCols()<<" mat.count="<<mat.count()<<" resize="<<resize);
  if( col<0 || col>=numCols() )
    return;
