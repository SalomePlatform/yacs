...
{
  DEBTRACE( "YACSGui_ViewItem::removeLinkItem " << text(0).latin1() );
  ...
  if ( aSLink || aSCLink )
  {
    ...
    while( aChildLinks )
    {
      DEBTRACE( ">> item : " << aChildLinks->text(0).latin1() );
...
void YACSGui_NodeViewItem::update(YACS::HMI::GuiEvent event, int type, YACS::HMI::Subject* son)
{
  ...
  case REMOVE:
	...
	DEBTRACE("NodeViewItem:  REMOVE link");
	removeLinkItem(son);
      }
      break;
    case INPUTPORT:
	...
	DEBTRACE("NodeViewItem:  REMOVE port");
	removePortItem(son);
      }
      break;
    case BLOC:
  ...
  case DOWN:
	...
	DEBTRACE("NodeViewItem:  DOWN port");
...
{
  DEBTRACE("YACSGui_NodeViewItem::addLinkItem");
  SubjectLink* aSLink = dynamic_cast<SubjectLink*>(theSLink);
  SubjectControlLink* aSCLink = dynamic_cast<SubjectControlLink*>(theSLink);
  if ( aSLink || aSCLink )
  {
...
void YACSGui_SchemaViewItem::update(YACS::HMI::GuiEvent event, int type, YACS::HMI::Subject* son)
{
  ...
  case ADD:
	...
	DEBTRACE("SchemaViewItem:  ADD node");
	addNodeItem(son);
      }
      break;
    case CONTAINER:
	...
	DEBTRACE("SchemaViewItem:  ADD container");
	addContainerItem(son);
      }
      break;
    case DATATYPE:
      {
	DEBTRACE("SchemaViewItem:  ADD data type");
	addDataTypeItem( son );
      }
      break;
    default:
  ...
  case REMOVE:
	...
	DEBTRACE("SchemaViewItem:  REMOVE container");
	...
	DEBTRACE("SchemaViewItem:  REMOVE component");
    ...
    case DATATYPE:
      {
	// remove a CORBA component item (this = schema item)
	DEBTRACE("SchemaViewItem:  REMOVE Data type");
	...
	DEBTRACE("SchemaViewItem:  REMOVE link");
	removeLinkItem(son);
      }
      break;
    case BLOC:
  ...
  case ADDCONTROLLINK:
    {
      ...
      DEBTRACE("SchemaViewItem:  ADDLINK");
      addLinkItem(son);
    }
    break;
  default:
...
{
  DEBTRACE("YACSGui_SchemaViewItem::addLinkItem");
  SubjectLink* aSLink = dynamic_cast<SubjectLink*>(theSLink);
  SubjectControlLink* aSCLink = dynamic_cast<SubjectControlLink*>(theSLink);
  if ( aSLink || aSCLink )
  {
...
void YACSGui_ComposedNodeViewItem::popup(YACSGui_Executor* anExecutor,const QPoint & point)
{
  ...
  if ( this == listView()->firstChild() )
    { //root schema object
      menu.insertItem("Schema container log",2);
    }
  int id=menu.exec(point);
  if(id==0)
    {
      std::string msg = anExecutor->getErrorDetails(getNode());
      LogViewer* log=new LogViewer(msg,listView(),"dialog",WDestructiveClose);
      log->setCaption("Error Details");
      log->show();
    }
  else if(id==1)
    {
      ...
      LogViewer* log=new LogViewer(msg,listView(),"dialog",WDestructiveClose);
      log->setCaption("Error Report");
      log->show();
    }
  else if(id==2)
    {
      std::string msg = anExecutor->getContainerLog();
      DEBTRACE(msg);
      LogViewer* log=new LogViewer(msg,listView(),"dialog",WDestructiveClose);
      log->readFile(msg);
      log->setCaption("Schema container log");
      log->show();
    }
}
...
void YACSGui_ElementaryNodeViewItem::popup(YACSGui_Executor* anExecutor,const QPoint & point)
{
  ...
  menu.insertItem("Error Details",0);
  menu.insertItem("Error Report",1);
  menu.insertItem("Node container log",2);
  int id=menu.exec(point);
  if(id==0)
    {
      std::string msg = anExecutor->getErrorDetails(getNode());
    ...
    {
      std::string msg;
      std::string file = anExecutor->getContainerLog(getNode());
      DEBTRACE(file);
      if(file != "")
        msg=file;
      else
        msg="This node has no container";
      LogViewer* log=new LogViewer(msg,listView(),"dialog",WDestructiveClose);
      log->setCaption("Node container log");
      if(file != "")
        log->readFile(file);
      log->show();
    }
