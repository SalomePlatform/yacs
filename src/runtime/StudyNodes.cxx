...
void StudyInNode::execute()
{
  ...
  if(CORBA::is_nil(obj)) 
    throw Exception("Execution problem: no naming service");
  ...
  for(iter = _setOfOutputPort.begin(); iter != _setOfOutputPort.end(); iter++)
    {
      ...
      if(CORBA::is_nil(aSO)) 
        {
          ...
          if(CORBA::is_nil(aSO)) 
            throw Exception("Execution problem: no id or path: " + data + " in study");
        }
      ...
      if ( aSO->FindAttribute( aGAttr, "AttributeIOR" ) )
        ...
        {
          //Problem !!!
          throw Exception("Execution problem: no AttributeIOR in study object: " + data );
        }
  ...
  if(CORBA::is_nil(myStudy))
    {
      ...
      if(CORBA::is_nil(myStudy))
        throw Exception("Execution problem: can not create new study " + msg.str());
    }
  DEBTRACE(myStudy->StudyId());

  SALOMEDS::StudyBuilder_var aBuilder =myStudy->NewBuilder() ;
  if(CORBA::is_nil(aBuilder))
    throw Exception("Execution problem: can not create StudyBuilder");
          ...
          if(CORBA::is_nil(aSO)) 
            {
              ...
              aSO=aBuilder->NewComponent("Container");
              if(CORBA::is_nil(aSO)) 
                {
